      ...
      <div class="shadow sm:rounded-md sm:overflow-hidden">
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
          <div class="grid grid-cols-6 gap-6">
            ...
            <div class="col-span-6 sm:col-span-6">
              <label for="short-description" class="block text-sm font-medium text-gray-700">Descripción breve de la empresa (max. 180 caracteres)</label>
            ...
            <div class="col-span-6 sm:col-span-3">
              ...
              <div class="mt-1 flex rounded-md shadow-sm">
                ...
                <input type="text" name="website" id="website" value="{{ isset($company->website) ? $company->website : '' }}" required class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300" placeholder="www.example.com">
              </div>  
            </div>
...
// Script that loads a preview of the selected image
...
function showImagePreview(){
    ...
    reader.onload = (e) => { 
      document.getElementById("preview-image-before-upload").src = e.target.result; 
    }
...
shortDescriptionInput.addEventListener("keyup",countCaracters);

function countCaracters(e){
  const max = 180;
  const textInput = e.target;
  let caracters = textInput.value.length;
  let left = max-caracters;
  const counterText = document.getElementById("short-description-counter");
  counterText.innerText = `${caracters}/${max} caracteres`;
  if(left < 0) {
